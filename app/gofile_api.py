...
class GofileClient:
    ...
    async def upload_file(self, file_path: str, folder_id: Optional[str] = None) -> Dict[str, Any]:
        """Upload a file, return API JSON (data dict)."""
        params = {}
        if folder_id:
            params["folderId"] = folder_id
        data = aiohttp.FormData()
        data.add_field("file", open(file_path, "rb"), filename=os.path.basename(file_path))
        async with self.session.post(UPLOAD_URL, data=data, params=params, headers=self._headers()) as resp:
            j = await resp.json(content_type=None)
            if resp.status != 200:
                return {"error": True, "status": resp.status, "response": j}
            # expected shape: { status: 'ok', data: {...} } (legacy) or just { data: {...} }
